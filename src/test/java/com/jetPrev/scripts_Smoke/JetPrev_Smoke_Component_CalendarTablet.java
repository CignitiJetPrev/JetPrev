...
public class JetPrev_Smoke_Component_CalendarTablet extends TestEngineWeb {
	...
	public void JetPrev_Component_CalendarTablet() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fncomponentcalendartablet(testData);
			if(blnbuymiles)
			{
		...
		JetPrev.fnUpdateDefectDetails(TestRunStatus.fnReportStatus(15, objIssue));
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] Calendar component styles in android tablet are not as expected";
		objIssue.DESCRIPTION="[Affiliate] Styles of calendar component in android tablet are not as expected in "+"testData.AppURL"+"URL";
