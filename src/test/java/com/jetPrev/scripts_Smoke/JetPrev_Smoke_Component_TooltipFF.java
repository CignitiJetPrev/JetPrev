...
public class JetPrev_Smoke_Component_TooltipFF extends TestEngineWeb {
	...
	public void JetPrev_Smoke_Component_TooltipFF() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnComponentHelptext(testData);
			if(blnbuymiles)
			{
		...
		//JetPrev.fnUpdateDefectDetails(TestRunStatus.fnReportStatus(7, objIssue));
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] ToolTip component styles in firefox are not as expected";
		objIssue.DESCRIPTION="[Affiliate] ToolTip component styles in firefox are not as expected in "+"testData.AppURL"+"URL";
