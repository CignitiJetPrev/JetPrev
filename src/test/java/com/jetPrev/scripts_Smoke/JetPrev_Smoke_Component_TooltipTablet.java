...
public class JetPrev_Smoke_Component_TooltipTablet extends TestEngineWeb {
	...
	public void JetPrev_Smoke_Component_TooltipTablet() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fncomponentHelpTextTablet(testData);
			if(blnbuymiles)
			{
				extentLogs.pass("JetPrev Component Tooltip ","Tooltip Component is present on : " + testData.AppURL );
				//First Row 
			...
			{
				extentLogs.fail("JetPrev Component Tooltip ","Tooltip Component is absent on : " + testData.AppURL );
			}
		...
		JetPrev.fnUpdateDefectDetails(TestRunStatus.fnReportStatus(14, objIssue));
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] ToolTip component styles in Android Tablet are not as expected";
		objIssue.DESCRIPTION="[Affiliate] ToolTip component styles in Android Tablet are not as expected in "+"testData.AppURL"+"URL";
