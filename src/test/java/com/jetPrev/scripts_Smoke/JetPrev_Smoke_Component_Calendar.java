...
public class JetPrev_Smoke_Component_Calendar extends TestEngineWeb {
	...
	public void JetPrev_Component_Calendar() throws Throwable {
		try {
			...
			for(int i=1;i<=4;i++) {
				...
				String strComponent = "";
				blnStatus = JetPrev.fnComponentCalendar(testData);
				strComponent = "Calendar";
				if(blnStatus) {
					System.out.println("For Iteration " + i + " Pass");
					extentLogs.pass("JetPrev Component Calendar ","Calendar Component is present on : " + testData.AppURL );

				}
				else {
					
					System.out.println("For Iteration " + i + " Fail");
