...
public class JetPrev_TC011_Lenskart extends TestEngineWeb {
	...
	public void ShopJetPrev_TC011_Lenskart() throws Throwable {
		try {
						
			testData=testData.getTestData(objDet);
			boolean blnJetPrevLogin=JetPrev.fnJetPrevLogin(testData);			
			//Lenskart
			boolean blnJetPrevLenskart		    =JetPrev.fnJetPrevRedirectToOnlinePartnerSiteLenskart();
			extentLogs.info("JetPrev Web Test Script ","JetPrev Lenskart redirection");
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] User is not able to validate URL for Lenskart partner";
		objIssue.DESCRIPTION="[Affiliate] User is not able to validate URL for Lenskart partner";
