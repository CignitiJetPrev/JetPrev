...
public class JetPrev_TC008_Myntra extends TestEngineWeb {
	...
	public void ShopJetPrev_TC008_Myntra() throws Throwable {
		try {
						
			testData=testData.getTestData(objDet);
			boolean blnJetPrevLogin=JetPrev.fnJetPrevLogin(testData);			
			//Myntra
			boolean blnJetPrevMyntra	=JetPrev.fnJetPrevRedirectToOnlinePartnerSiteMyntra();
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] User is not able to validate URL for Myntra partner";
		objIssue.DESCRIPTION="[Affiliate] User is not able to validate URL for Myntra partner";
