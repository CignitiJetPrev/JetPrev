...
public class JetPrev_TC007_UrbanLadder extends TestEngineWeb {
	...
	public void ShopJetPrev_TC007_UrbanLadder() throws Throwable {
		try {
						
			testData=testData.getTestData(objDet);
			boolean blnJetPrevLogin=JetPrev.fnJetPrevLogin(testData);			
			//UrbanLadder
			boolean blnJetPrevUrbanLadder		    =JetPrev.fnJetPrevRedirectToOnlinePartnerSiteUrbanLadder();
			extentLogs.info("JetPrev Web Test Script ","JetPrev Urban Ladder redirection");
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] User is not able to validate URL for Urban Ladder partner";
		objIssue.DESCRIPTION="[Affiliate] User is not able to validate URL for Urban Ladder partner";
