...
public class JetPrev_TC010_TataCliq extends TestEngineWeb {
	...
	public void ShopJetPrev_TC010_TataCliq() throws Throwable {
		try {
						
			testData=testData.getTestData(objDet);
			boolean blnJetPrevLogin=JetPrev.fnJetPrevLogin(testData);			
			//Tata Cliq
			boolean blnJetPrevTataCliq	=JetPrev.fnJetPrevRedirectToOnlinePartnerSiteTataCliq();
		...
		JetPrev.fnUpdateDefectDetails(TestRunStatus.fnReportStatus(10, objIssue));
	}
	private void fnGetIssueDetails()
	{
		objIssue.ASSIGNEE="admin";
		objIssue.PROJECT="CIG";
		objIssue.SUMMARY="[Affiliate] User is not able to validate URL for Tata Cliq partner";
		objIssue.DESCRIPTION="[Affiliate] User is not able to validate URL for Tata Cliq partner";
