...
public class JetPrev_Smoke_AndroidTabletComponent_PDFPrint extends TestEngineWeb {
	...
	public void JetPrev_Smoke_AndroidTabletComponent_PDFPrint() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnComponentPDFandPRINT(testData);
			if(blnbuymiles)
			{
			...
			{
				extentLogs.fail("JetPrev Component PDFPrint ","PDFPrint Component is absent on : " +" 'testData.AppURL'" );
			}
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] PDFPrint component styles on android tablet browser chrome are not as expected";
		objIssue.DESCRIPTION="[Affiliate] Styles of PDFPrint component on android tablet browser are not as expected in "+"testData.AppURL"+"URL";
