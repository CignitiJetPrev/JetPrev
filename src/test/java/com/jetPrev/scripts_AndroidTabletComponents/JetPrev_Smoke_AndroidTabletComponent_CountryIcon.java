...
public class JetPrev_Smoke_AndroidTabletComponent_CountryIcon extends TestEngineWeb {
	...
	public void JetPrev_Smoke_AndroidTabletComponent_CountryIcon() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnComponentCountryIcon(testData);
			if(blnbuymiles)
			{
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate]  CountryIcon component styles on android tablet are not as expected";
		objIssue.DESCRIPTION="[Affiliate]  Styles of countryicon component in android tablet browser are not as expected in "+"testData.AppURL"+"URL";
