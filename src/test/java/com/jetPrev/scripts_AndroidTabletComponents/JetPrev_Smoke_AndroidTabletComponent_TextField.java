...
public class JetPrev_Smoke_AndroidTabletComponent_TextField extends TestEngineWeb {
	...
	public void JetPrev_Smoke_AndroidTabletComponent_TextField() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnComponentTextField(testData);
		//	boolean blnbuymiles=JetPrev.fnComponentsWebaccesibility(testData);
			if(blnbuymiles)
			{
				extentLogs.pass("JetPrev Component TextField ","TextField Component is absent on : " + testData.AppURL);
				//First Row 
			...
			{
				extentLogs.fail("JetPrev Component TextField ","TextField Component is absent on : " + testData.AppURL );
			}
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] TextField component styles in android tablet chrome are not as expected";
		objIssue.DESCRIPTION="[Affiliate] Styles of TextField component in android tablet browser are not as expected in "+"testData.AppURL"+"URL";
