...
public class JetPrev_Smoke_AndroidTabletComponent_Primarybutton extends TestEngineWeb {
	...
	public void JetPrev_Smoke_AndroidTabletComponent_Primarybutton() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnComponentPrimaryButton(testData);
			if(blnbuymiles)
			{
			...
			{
				extentLogs.fail("JetPrev Component primaryButton ","PrimaryButton Component is absent on : " + testData.AppURL );
			}
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] Calendar component styles in android tablet browser are not as expected";
		objIssue.DESCRIPTION="[Affiliate] Styles of calendar component in android tablet browser are not as expected in "+"testData.AppURL"+"URL";
