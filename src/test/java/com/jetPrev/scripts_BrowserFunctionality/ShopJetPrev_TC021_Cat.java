...
public class ShopJetPrev_TC021_Cat extends TestEngineWeb {
	...
	public void shopjetPrev_TC021_Cat() throws Throwable {
		try {
			...
			boolean blncat=JetPrev.fncat(testData);
			if(blncat)
			{
				extentLogs.pass("JetPrev Web Test Script ","Redirection to the Electronics and Gaming screen when user selects Electronics and Gaming from the All Categories list Pass");
...
//			}
		...
		JetPrev.fnUpdateDefectDetails(TestRunStatus.fnReportStatus(21, objIssue));//Change issue no  --- 
	}
		...
		private void fnGetIssueDetails()
		{
			...
			objIssue.SUMMARY="[Affiliate] User is not able to redirect to the Electronics and Gaming screen when selects Electronics and Gaming from the All Categories list ";
			objIssue.DESCRIPTION="[Affiliate] User is not able to validate when selects Electronics and Gaming from the All Categories list ";
