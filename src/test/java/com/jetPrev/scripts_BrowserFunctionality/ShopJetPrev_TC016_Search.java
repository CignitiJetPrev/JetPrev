...
public class ShopJetPrev_TC016_Search extends TestEngineWeb {
	...
	public void shopJetPrev_TC016_Search() throws Throwable {
		try {
			...
			//	testData=testData.getTestData(objDet);
				...
				JetPrev.fnJetPrevSearchbar(JetPrevPageLocators.retailPartners, JetPrevPageLocators.proceedToLyfstyle);
		...
		TestCaseStatus(testCaseStatus, testCaseFailureReason);
		JetPrev.fnUpdateDefectDetails(TestRunStatus.fnReportStatus(16, objIssue));//Change issue no  --- 
	...
	//add this block
		private void fnGetIssueDetails()
		{
			...
			objIssue.SUMMARY="[Affiliate] User is not able to place an order by searching the product from Other Retail Partners";
			objIssue.DESCRIPTION="[Affiliate] User is not able to place an order by searching the product from Other Retail Partners";
