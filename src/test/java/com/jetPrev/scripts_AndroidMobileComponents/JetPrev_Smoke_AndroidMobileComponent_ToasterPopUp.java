...
public class JetPrev_Smoke_AndroidMobileComponent_ToasterPopUp extends TestEngineWeb {
	...
	public void JetPrev_Smoke_AndroidMobileComponent_ToasterPopUp() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnComponentToasterPopup(testData);
			if(blnbuymiles)
			{
			...
			{
				extentLogs.fail("JetPrev Component ToasterPopUp ","ToasterPopUp Component is absent on : " + testData.AppURL );
			}
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] ToasterPopUp component styles on android mobile browser are not as expected";
		objIssue.DESCRIPTION="[Affiliate] Styles of ToasterPopUp component on android mobile browser are not as expected in "+"testData.AppURL"+"URL";
