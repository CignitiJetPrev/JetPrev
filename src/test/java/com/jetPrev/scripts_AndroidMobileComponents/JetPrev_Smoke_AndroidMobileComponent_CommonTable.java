...
public class JetPrev_Smoke_AndroidMobileComponent_CommonTable extends TestEngineWeb {
	...
	public void JetPrev_Smoke_AndroidMobileComponent_CommonTable() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnComponentCommonTable(testData);
			if(blnbuymiles)
			{
			...
			{
				extentLogs.fail("JetPrev Component CommonTable ","CommonTable Component is absent on : " + testData.AppURL );
			}
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] CommonTable component styles on android mobile browser are not as expected";
		objIssue.DESCRIPTION="[Affiliate] Styles of CommonTable component on android mobile browser are not as expected in "+"testData.AppURL"+"URL";
