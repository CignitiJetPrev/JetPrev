...
public class JetPrev_Smoke_AndroidMobileComponent_PostLoginNotification extends TestEngineWeb {
	...
	public void JetPrev_Smoke_AndroidMobileComponent_PostLoginNotification() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnComponentpostNotifications(testData);
			if(blnbuymiles)
			{
			...
			{
				extentLogs.fail("JetPrev Component PostLoginNotification ","PostLoginNotification Component is absent on : " + testData.AppURL );
			}
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] Calendar component styles in android chrome mobile are not as expected";
		objIssue.DESCRIPTION="[Affiliate] Styles of calendar component in android mobile chrome browser are not as expected in "+"testData.AppURL"+"URL";
