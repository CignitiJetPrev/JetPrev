...
public class JetPrev_Smoke_AndroidMobileComponent_BreadcrumPresentation extends TestEngineWeb {
	...
	public void JetPrev_Smoke_AndroidMobileComponent_BreadcrumPresentation() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnbreadcrumpresentation(testData);
			if(blnbuymiles)
			{
			...
			{
				extentLogs.fail("JetPrev Component BreadcrumPresentation ","BreadcrumPresentation Component is absent on : " + testData.AppURL );
			}
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] BreadcrumPresentation component styles on android mobile browser are not as expected";
		objIssue.DESCRIPTION="[Affiliate] Styles of BreadcrumPresentation component android mobile browser are not as expected in "+"testData.AppURL"+"URL";
