package com.jetPrev.scripts_AndroidMobileComponents;
...
public class JetPrev_Smoke_AndroidMobileComponent_ActionButtonExternalURL extends TestEngineWeb {
	private String testCaseFailureReason = "";
	private boolean testCaseStatus = true;
	private ExtentLogs extentLogs = new ExtentLogs();
	public IssueDetails objIssue=new IssueDetails();
	private JetPrevInput testData=new JetPrevInput();
	private JetPrevWorkflowsMobile JetPrev;
	...
	public void  beforeMethod(int rowNum) throws Throwable {
		JetPrev = new JetPrevWorkflowsMobile(CommonVariables.CommonDriver.get());
	...
	@Test(description = "JetPrev UAT Environment", groups = { "smoke", "regression" })
	public void JetPrev_Smoke_AndroidMobileComponent_ActionButtonExternalURL() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnComponentActionButtonExternalURL(testData);
			if(blnbuymiles)
			{
			...
			{
				extentLogs.fail("JetPrev Component ActionButtonExternalURL ","ActionButtonExternalURL Component is absent on : " + testData.AppURL );
			}
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] ActionButtonExternalURL component styles in mobile are not as expected";
		objIssue.DESCRIPTION="[Affiliate] Styles of ActionButtonExternalURL component in mobile browser are not as expected in "+"testData.AppURL"+"URL";
