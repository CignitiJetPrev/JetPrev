...
public class JetPrev_Smoke_AndroidMobileComponent_PasswordText extends TestEngineWeb {
	...
	public void JetPrev_Smoke_AndroidMobileComponent_PasswordText() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnComponentPasswordText(testData);
			if(blnbuymiles)
			{
			...
			{
				extentLogs.fail("JetPrev Component Passwordtext ","Passwordtext Component is absent on : " + testData.AppURL );
			}
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] passwordtext component styles in chrome are not as expected";
		objIssue.DESCRIPTION="[Affiliate] Styles of passwordtext component in chrome browser are not as expected in "+"testData.AppURL"+"URL";
