...
public class JetPrev_Smoke_AndroidMobileComponent_ContentLevelTab extends TestEngineWeb {
	...
	public void JetPrev_Smoke_AndroidMobileComponent_ContentLevelTab() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnComponentContentLevelTab(testData);
			if(blnbuymiles)
			{
			...
			{
				extentLogs.fail("JetPrev Component ContentLevelTab ","ContentLevelTab Component is absent on : " + testData.AppURL );
			}
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] ContentLevelTab component styles on android mobile browser are not as expected";
		objIssue.DESCRIPTION="[Affiliate] Styles of ContentLevelTab component on android mobile browser are not as expected in "+"testData.AppURL"+"URL";
