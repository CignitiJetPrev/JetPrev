...
public class JetPrev_Smoke_AndroidMobileComponent_P extends TestEngineWeb {
	...
	public void JetPrev_Smoke_AndroidMobileComponent_P() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnComponentP(testData);
			if(blnbuymiles)
			{
			...
			{
				extentLogs.fail("JetPrev Component P ","P Component is absent on : " + testData.AppURL );
			}
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] P component styles on android mobile browser are not as expected";
		objIssue.DESCRIPTION="[Affiliate] Styles of P component on android mobile browser are not as expected in "+"testData.AppURL"+"URL";
