...
public class JetPrev_Smoke_AndroidMobileComponent_NewFormLabel extends TestEngineWeb {
	...
	public void JetPrev_Smoke_AndroidMobileComponent_NewFormLabel() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnComponentNewFormLabel(testData);
			if(blnbuymiles)
			{
			...
			{
				extentLogs.fail("JetPrev Component NewFormLabel ","NewFormLabel Component is absent on : " + testData.AppURL );
			}
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] NewFormLabel component styles on android mobile browser are not as expected";
		objIssue.DESCRIPTION="[Affiliate] Styles of NewFormLabel component in android mobile browser are not as expected in "+"testData.AppURL"+"URL";
