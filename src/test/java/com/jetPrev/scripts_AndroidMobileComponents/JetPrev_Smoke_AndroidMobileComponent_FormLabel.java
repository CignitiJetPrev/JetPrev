...
public class JetPrev_Smoke_AndroidMobileComponent_FormLabel extends TestEngineWeb {
	...
	public void JetPrev_Smoke_AndroidMobileComponent_FormLabel() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnComponentoldFormLabelupdated(testData);
			if(blnbuymiles)
			{
			...
			{
				extentLogs.fail("JetPrev Component FormLabel ","FormLabel Component is absent on : " + testData.AppURL );
			}
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] FormLabel component styles on android mobile browser are not as expected";
		objIssue.DESCRIPTION="[Affiliate] Styles of FormLabel component on android mobile browser are not as expected in "+"testData.AppURL"+"URL";
