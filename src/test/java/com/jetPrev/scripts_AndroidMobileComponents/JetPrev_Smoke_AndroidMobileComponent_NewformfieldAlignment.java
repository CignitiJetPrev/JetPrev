...
public class JetPrev_Smoke_AndroidMobileComponent_NewformfieldAlignment extends TestEngineWeb {
	...
	public void JetPrev_Smoke_AndroidMobileComponent_NewformfieldAlignment() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnComponentNewFormContainer(testData);
			if(blnbuymiles)
			{
			...
			{
				extentLogs.fail("JetPrev Component NewformfieldAlignment ","NewformfieldAlignment Component is absent on : " + testData.AppURL );
			}
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] NewformfieldAlignment component styles on android mobile browser are not as expected";
		objIssue.DESCRIPTION="[Affiliate] Styles of NewformfieldAlignment component styles on android mobile browser are not as expected in "+"testData.AppURL"+"URL";
