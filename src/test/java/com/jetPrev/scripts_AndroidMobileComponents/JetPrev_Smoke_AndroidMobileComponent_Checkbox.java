...
public class JetPrev_Smoke_AndroidMobileComponent_Checkbox extends TestEngineWeb {
	...
	public void JetPrev_Smoke_Component_Checkbox() throws Throwable {
		try {
			...
			boolean blnbuymiles=JetPrev.fnComponentCheckbox(testData);
			//boolean blnbuymiles=JetPrev.fnComponentCheckboxActive(testData);
			if(blnbuymiles)
			{
			...
			{
				extentLogs.fail("JetPrev Component CheckboxInactive ","CheckboxInactive Component is present on : " + testData.AppURL );
			}
	...
	private void fnGetIssueDetails()
	{
		...
		objIssue.SUMMARY="[Affiliate] CheckboxInActive component styles in chrome are not as expected";
		objIssue.DESCRIPTION="[Affiliate] Styles of CheckboxInActive component in chrome browser are not as expected in "+"testData.AppURL"+"URL";
